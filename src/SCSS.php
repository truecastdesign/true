...
class scss_parser {
	...
	public function throwParseError($msg = "parse error", $count = null) {
		...
		if ($this->peek("(.*?)(\n|$)", $m, $count)) {
			trigger_error("$msg: failed at `$m[1]` $loc",16384);#throw new Exception("$msg: failed at `$m[1]` $loc");
		} else {
			trigger_error("$msg: $loc",16384);#throw new Exception("$msg: $loc");
		}
	}
