...
class JWT 
{
	...
	*
	*/
	public function encode($payload, string $key, string $keyPassword, $alg = 'RS256', $keyId = null, $head = null)
	{
		...
		$signature = static::sign($signing_input, $key, $keyPassword, $alg);
	...
	* @param string $key    pem formatted private key file path
	* @param string $keyPassword    The private key password
	* @param string $alg    The signing algorithm. Supported algorithms are RS256, RS384, RS512
	*
	...
	{ 
		...
		switch($function) {
			case 'openssl':
					...
					if (!$success)
						throw new \Exception("OpenSSL unable to sign data. ".openssl_error_string());
