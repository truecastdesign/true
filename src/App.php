...
class App
{
	...
	public function get($pattern, $callable, array $passedVars = [])
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('GET', $pattern, [], $callable, $passedVars);
		} else {
	...
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('POST', $pattern, $callable, $passedVars, $extra);
		} else {
	...
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('PUT', $pattern, $callable, $passedVars, $extra);
		} else {
	...
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('PATCH', $pattern, $callable, $passedVars, $extra);
		} else {
	...
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('DELETE', $pattern, $callable, $passedVars, $extra);
		} else {
	...
	{
		if (substr($pattern, 0, 4 ) == "http") {
			$this->request('OPTIONS', $pattern, $callable, $passedVars, $extra);
		} else {
    ...
    *	}, [ new \App\AuthMiddleware ]);
	...
	public function controller($path)
	{
		...
		if (\True\Functions::startsWith($path, BP)) {
			return $path;
		} else {
